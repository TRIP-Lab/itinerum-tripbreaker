...
def find_metro_transfers(stations, segment_groups, buffer_m):
    ...
    for num, segments in segment_groups.iteritems():
...
def connect_by_velocity(linked_trips):
    ...
    last_trip = None
    for num, trip in linked_trips.iteritems():
        if not last_trip:
...
def filter_single_points(linked_trips):
    ...
    max_time = 20
    max_dist = 150
    for idx, (num, trip) in enumerate(test_trips.iteritems()):
...
def merge_trips(trips, missing_trips, stations):
    ...
    rows = []
    offset = 0
    for idx, trip in trips.iteritems():
        note = None
...
def summarize(rows):
    ...
    summaries = {}
    for num, trip in trips.iteritems():
        ...
        direct_distance = tools.pythagoras((start_pt['easting'], start_pt['northing']), (end_pt['easting'], end_pt['northing']))
